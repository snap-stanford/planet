...
class HGTConv(MessagePassing):
    def __init__(self, in_dim, out_dim, num_relations, num_types, n_heads, dropout = 0.2, use_norm = True, use_RTE = True, num_bases=None, **kwargs):
        if 'aggr' in kwargs:
            kwargs.pop('aggr')
    ...
    def forward(self, node_inp, edge_index, edge_type, node_type=None, edge_time=None):
        if node_type is None:
            # print ('node_inp[0].size()', node_inp[0].size())
            # print ('node_inp[1].size()', node_inp[1].size())
            node_type = (node_inp[0].new_full((node_inp[0].size(0),), 0), node_inp[1].new_full((node_inp[1].size(0),), 0))
    ...
    def update(self, aggr_out, node_inp, node_type):
        ...
        # print ('type(node_inp)', type(node_inp))
        # print ('type(node_type)', type(node_type))
        # print ('aggr_out.size()', aggr_out.size())
        # print ('node_inp[0].size()', node_inp[0].size())
        # print ('node_inp[1].size()', node_inp[1].size())
        if isinstance(node_inp, tuple):
            node_inp = node_inp[1]
