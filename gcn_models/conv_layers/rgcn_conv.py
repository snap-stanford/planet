...
class RGCNConv(MessagePassing):
        ...
        if self.num_blocks is not None:  # Block-diagonal-decomposition =====
            ...
            for i in range(self.num_relations):
                ...
                if x_l.dtype == torch.long:
                    ...
                    h = self.propagate(tmp, x=x_l, size=size)
                    if h.isnan().sum():
                        print (f'RGCN: self.propagate(tmp, x=x_l, size=size) gives NaN!')
                    out = out + (h @ weight[i])
                    if out.isnan().sum():
                        print (f'RGCN: out = out + (h @ weight[i]) gives NaN!')
    ...
    def message_and_aggregate(self, adj_t: SparseTensor, x: Tensor) -> Tensor:
        ...
        if adj_t.isnan().sum():
            print (f'RGCN: adj_t gives NaN!')
        out = matmul(adj_t, x, reduce=self.aggr)
        if out.isnan().sum():
            print (f'RGCN: out = matmul(adj_t, x, reduce=self.aggr) gives NaN!')
        return out
